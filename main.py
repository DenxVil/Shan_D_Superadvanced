...
class ShanDApplication:
    ...
    async def initialize(self) -> bool:
        try:
            ...
            # Setup integrations (including Telegram)
    ...
    async def _initialize_core_components(self):
        ...
        try:
            ...
            self.components = {name: None for name in [
                'emotion_engine', 'memory_manager',
                'learning_engine', 'model_manager',
                'conversation_flow', 'multimodal_processor'
            ]}
        except Exception as e:
    ...
    async def _initialize_ai_models(self):
        ...
        try:
            ...
            if model_manager and hasattr(model_manager, 'initialize_models'):
                await model_manager.initialize_models(self.config_manager.config.get('api_keys', {}))
            ...
            try:
                if self.components.get('conversation_flow'):
                    response = await self.components['conversation_flow'].process_message(
                        message.get('text', ''), message.get('user_id', 'anonymous'), message.get('context', {})
                    )
                else:
                    response = {
...
async def main():
    ...
    # Keep alive so both API and Telegram bot run
    try:
        while True:
